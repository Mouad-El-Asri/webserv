...
std::string* handle_cgi(t_client_info *client, Directives& working, int flag)
{
    ...
    if (flag == 0)
    {
        ...
        if (pid == 0)
        {
            ...
            Content_Type = get_header_value(client, "Content-Type", 14);
            Content_Length = get_header_value(client, "Content-Length", 16);
            Path_Info = grab_path(client->fst_req);
            Server_Name = working.getServerName();
